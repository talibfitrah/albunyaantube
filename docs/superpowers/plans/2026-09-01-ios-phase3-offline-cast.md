...
4. **`RequestKind` has no download lane and Android's never did.** InnerTubeKit's enum maps "1:1" to Android's (`ExtractionRateLimiter.swift:3-8`) — true, because Android's `DownloadWorker` never touches the rate limiter at all (it resolves via `repository.resolveStreams` with a `USER_FOREGROUND` declaration, `DownloadWorker.kt:324-336`). So "Android parity" answers nothing here. Reconciliation note 4 decides the lane (default: reuse `.prefetch`, fork B).
...
- Modify: `backend/src/main/java/com/albunyaan/tube/controller/RegistryController.java` (`updateVideo`, `:981` — copy the field when present, audit-log it)
...
- Modify: `frontend/src/views/ContentLibraryView.vue` + `frontend/src/services/contentLibrary.ts` (a "Save for offline" toggle on the video edit affordance, wired to the existing `PUT /api/admin/registry/videos/{id}`), `frontend/src/locales/messages.ts` (en/ar/nl labels)
- Create/modify tests: backend controller/service tests; a Vitest for the toggle wiring

**Red tests first:**
- Backend: `updateVideo` with `offlineAllowed: true` persists it; absent field leaves it unchanged; `getVideoDetails` response carries the field; a legacy document with no field reads as false/absent (Firestore null → the iOS client's absent→false rule holds end-to-end).
- Frontend: toggling calls the update service with the field; the toggle renders only for videos (not channels/playlists).

**Steps:**
- [ ] Write the failing backend tests → implement model + controller copy + audit log entry (`video_updated_in_registry` already fires; ensure the field is in the copied set).
...
- Resolve path: `rateLimiter.check(videoId, kind: .prefetch, now: clock.now)` (`InnerTube.rateLimiter`/`InnerTube.clock` — reuse the ONE clock, `InnerTube.swift:17-20`); `.delayed/.blocked` → stay `queued`, schedule retry at `retryAfter` (reconciliation note 4); `.allowed` → `resolver.resolve(videoId, purpose: .prefetch, sourceChannelId: nil, forceRefresh: false)`. `.embed` outcome → `failed(notSaveable)` terminal. `.hls`/`.progressive` → check `expiresAt` leaves ≥10 min, else re-resolve `forceRefresh: true` once; hand to the engine with the resolved `userAgent` as a header.
...
- Modify: `ios/FitrahTube/App/Route.swift` (add `case offline`), `ios/FitrahTube/Features/Placeholders/PhaseTwoPlaceholderView.swift` (both exhaustive switches), `ios/FitrahTube/Features/Shell/MainShellView.swift` (explicit `case .offline` arm ABOVE the `default:` — the silent-placeholder trap, Global Constraints)
...
**Red tests first:**
- The `MainShellView` arm pin (a `Route.offline` destination is `SavedScreen`, not the placeholder — the Plan C Task 4/5 idiom).
