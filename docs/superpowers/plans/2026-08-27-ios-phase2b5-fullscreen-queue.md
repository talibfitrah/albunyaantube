...
- That is also what makes **background auto-advance** work: with the screen locked and the itag 140 rendition playing, the end-of-item notification fires, the advance resolves from the warm `ManifestCache` and the next `updateUIViewController` pass swaps the item on the live player — no host teardown, but it DOES depend on SwiftUI running an update pass while suspended (CF-B5-k; the synchronous-swap pattern C2 uses for the audio-only swap is the remedy if the device confirms it never runs). A `.loading` hop would dismantle the host, `detach()` the audio session mid-background, and end playback. **This is the reason `showLoading: false` on the advance is load-bearing, not a preference.**
...
- **CF-B5-8:** the double-tap recognizer lives on `AVPlayerViewController.view` (Task 3 finding: `contentOverlayView` sits below AVKit's controls layer and never fires). If a later plan puts a hit-testable SwiftUI overlay above the host, single taps stop reaching AVKit and the stock controls stop toggling. Any new player overlay must be `.allowsHitTesting(false)` unless it deliberately owns its region.
