...
| **3** | **Single videos path, provenance flag dropped** — the dual path works around NewPipe v0.26 bugs iOS does not inherit | 2 and reconciliation note 1. One call: `BrowseClient.channelVideos`. No `videosUseChannelTab`, no cursor-family tracking, no channel-tab fallback. **Outcome (Task 6, 07ce6f16):** the `VLUU…` uploads playlist capped at 200 rows live (100 + 100, then no continuation) while the Videos-tab `richGridRenderer` continuation paged 30/page past 300 — `channelVideos` now browses the Videos tab; still one path, no provenance flag. Behaviour change: the tab excludes Shorts/streams, which the Live and Shorts tabs cover. |
...
|---|---|---|---|---|
| Videos | `channelVideos` (Videos-tab `richGridRenderer` continuation, ruling 3 outcome — `VLUU…` capped at 200 live) | `VideoRow` | `Route.player(PlayerArgs(…, channelId: id, channelName: header.name))` | `channel_videos_empty` |
...
2. **Deep pagination**: scroll the Videos tab of a channel with 500+ uploads past page 5 and confirm the `VLUU…` continuation keeps returning items. This is the measurement reconciliation note 1 defers to; if it caps at ~100, report it — the fix is ruling 3's named fallback, with evidence. **Measured 2026-08-30:** capped at 200 (100 + 100, continuation nil); the Videos-tab continuation paged 30/page past 300 → single-path swap in `07ce6f16`, live leg asserts >200 distinct rows.
...
- **CF-C-3:** the docs disagree with the ruling about which continuation surface to page (reconciliation note 1). `ios-app-plan.md:179` and spec §9 say `VLUU…`; Android abandoned it in v0.26.3. Plan C shipped `VLUU…` per ruling 3 and measured it in Task 6: it caps at 200 rows live, so `07ce6f16` swapped `channelVideos` to the Videos-tab `richGridRenderer` continuation (30/page, paged past 300). The docs' conclusion was wrong for iOS as well as stale; `ios-app-plan.md` and spec §9 amended 2026-08-30.
