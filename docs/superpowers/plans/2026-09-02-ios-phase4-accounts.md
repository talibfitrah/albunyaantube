...
- **CF-A-59 (HIGH, found by the stage-5 review at Task 42, 2026-09-23, PRE-EXISTING — not introduced):** a save can still outlive a device wipe. `OfflineManager.save` suspends at `await read(videoId:)` before its only `store.insert`; a save parked there is missed by `cancelAll()` AND by the id snapshot taken after it, then inserts while `deleteAll` is awaiting. The row and its file survive, `wipe()` returns nil and the caller redeems the marker — the deleted account's saved video reaches the next user. Reachable because the cleanup is detached (CF-G-5) and the 403-envelope path can fire from any screen. Task 42's reorder NARROWS the window; it does not close it, and the comment there says so. Fix direction (reviewer's, not implemented): a synchronous teardown counter bumped by `cancelAll()` and captured by `save()` at entry, which refuses its `write { insert }` if it changed — a barrier in the one inserter; widening the snapshot cannot work.
- **CF-A-60 (LOW, same review):** the refusal's `.failedUnknown` banner may never be seen — the same sign-out that empties `currentUid` drives `ProfileScreen`'s `.signedOut` dismiss and the root's signed-out shell. Safe either way (nothing is owed); the state is correct, the presentation is not guaranteed. Hoisting the banner to the root would fix it.
