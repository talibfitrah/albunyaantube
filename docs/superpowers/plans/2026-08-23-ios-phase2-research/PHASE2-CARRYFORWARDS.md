...
B2 shipped 2026-08-27 (`5e2fafa7..f3916ddd`: itag 140 + live manifest TTL, audio session/interruptions/routes, background-play + audio-only, Now Playing + remote commands, PiP, rate-limiter gate + pre-emptive foreground re-resolve, acceptance pass). Open items its successors must handle:
...
- **CF-B2-7** `BackgroundPlaybackController.willEnterForeground()` **awaits** `onWillEnterForeground` before running the foreground policy, deliberately: `.restoreVideo` must rebuild from the freshly resolved stream, not the expiring one. That makes the closure a blocking step on every foreground transition. B3 must not chain slow work onto it (an embed reload, a network round trip with no timeout) — anything that isn't the TTL refresh belongs after `handle(.willEnterForeground)`, not inside the awaited closure. → Plan B3
...
**Do not treat B2 as verified until the 26-item device checklist in `docs/superpowers/plans/2026-08-27-ios-phase2b2-background-audio.md` (Task 7, Step 3) has been run on real hardware.** Any plan that lands on top of B2 inherits that debt.
