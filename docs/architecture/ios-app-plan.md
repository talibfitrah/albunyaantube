...
5. Ships **every Android feature** (§7) in two releases: v1 = everything usable without an account; v1.1 = accounts, Me tab, sync, submissions, import. In-app update does not port (App Store rules); Chromecast becomes AirPlay. Downloads port in the constrained "Save for offline" shape (owner ruling 2026-09-01 — see the §9 checklist line): in-app sandboxed only, per-video admin-gated, remote-config kill-switch reviewed ON.
...
│     └── strategies       → visionosHLS | androidItag18 | embed (implemented; ships dark — not in the published resolverOrder)  (Owner directive 2026-08-27: no openInYouTube rung)
...
- `BrowseClient`: InnerTube `browse` with the WEB client context and its own `visitorData`; Videos via the channel's Videos-tab `richGridRenderer` continuation (measured 2026-08-30: 30/page past 300; the `VLUU…` uploads-playlist trick Android uses caps at 200 rows for iOS's WEB-context client), tabs via channel `params` for Live / Shorts / Playlists, playlist items via `VL<playlistId>`; apply the backend's exclusions; push items to `/api/v1/index/streams` like Android.
...
| Downloads screen, download quality / Wi-Fi-only / storage settings, download notifications, `api/downloads/*` | **Ported as "Save for offline"** (owner ruling 2026-09-01, §9 checklist shape): app-container storage only, no export/share/Photos/Files exposure, in-app playback only, per-video `offlineAllowed` backend gate, `downloadsEnabled` config kill-switch reviewed ON. Links to Favorites / history still move to the Me tab | Phase 3 |
...
- [ ] Offline saving (owner ruling 2026-09-01, supersedes the earlier "no downloads" line): in-app sandboxed only — app container, `isExcludedFromBackup`, no `UIFileSharingEnabled`/`LSSupportsOpeningDocumentsInPlace`, no share sheet/Photos/export of media, playback in-app only; user-facing name "Save for offline", never "Download" (strings, listing, screenshots). Gated per-video by backend `offlineAllowed` (admin-set, default false; creator-authorization records kept ready for 5.2.3's "authorization provided upon request" exchange — that is Apple's own documented remedy path). `downloadsEnabled` remote-config kill-switch: ON during App Review and described with specificity in Notes for Review (2.3.1 — never dark past review; the flag exists to turn a reviewed feature OFF on complaint, same-day, alongside `resolverOrder: ["embed"]`). Saving uses the same resolver ladder as playback; no offline from the embed rung; auto-delete on catalog removal (§8 revalidation). Evidence base and residual risk: sandboxed offline raises removal-on-complaint risk marginally, not categorically (Musi was removed with NO download feature — complaints drive removal; CapTik/LaterPlay/Smart Tube BDP ship YouTube-saving or Downloads tabs live today); the categorical risk was accepted with the extraction spine. Never pair offline copy with "ad-free" — that pairing is the Musi/ProTube complaint profile.
...
- [ ] Reviewer notes (2.3.1): curated library selected by an editorial team; content is the creators' public videos; works without an account; demo path that plays during review; the player is described truthfully — native playback with YouTube's embedded player as fallback, which shows YouTube's ads and branding — never the embed as *the* playback method. Embed rung added to the published `resolverOrder` before App Review submission (it ships dark today — the published config lists visionosHLS and androidItag18 only) so a blocked review IP still sees a working player.
