...
- **Downloads**: WorkManager-powered queue (`DownloadWorker` + foreground notifications) manages offline requests via `DownloadRepository`, exposing pause/resume/cancel hooks. Actual media writing remains stubbed until storage policy finalizes; downloads target app-private storage with quotas (see [`../security/threat-model.md`](../security/threat-model.md#policy-controls)).
