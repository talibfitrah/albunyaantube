...
- **Metadata Pipeline**: `RetrofitContentService` now routes list payloads through `MetadataHydrator`, which invokes a (stubbed) NewPipeExtractor client to merge Albunyaan overrides with hydrated titles, thumbnails, and stats before data flows into Paging 3 caches. Failures fall back to existing fake data so list UX remains responsive while telemetry will flag extractor regressions once real wiring lands.
...
- **Playback (Phase 8 scaffold)**: `PlayerFragment` hosts ExoPlayer with an audio-only toggle, status bar, and an "Up Next" queue fed by `PlayerViewModel`. The ViewModel now owns queue state, applies exclusion filtering ahead of playback, surfaces analytics debug events for start/complete/toggle actions, and will be the integration point once `/videos/{id}` + `/next-up` payloads land. Media sourcing remains stubbed pending backend wiring, but event hooks mirror the future analytics contract.
