...
**Status**: ✅ **Implemented** (P0–P3 functional; manual deployment test pending; P4 metrics deferred)
...
- [ ] **TTL semantics**: `openedAt` and `cooldownUntil` should use Firestore server timestamps (currently uses `Instant.now()` - see Known Limitations).
...
- [x] **Never-archive guarantee on transient failures**:
  ...
  - [x] Unit test: Simulate rate-limit mid-batch; confirm skipped items not saved, only processed items updated (`MidBatchSkippedItemsTest`).
...
### All Phases
- [x] Tests pass within 300s: `cd backend && timeout 300 ./gradlew test`

---

## 8) Known Limitations

### Firestore Server Timestamps Not Implemented

**Issue**: The plan specifies using `FieldValue.serverTimestamp()` for `openedAt` and `cooldownUntil` to avoid clock skew between backend instances. The current implementation uses `Instant.now()` (client-side timestamps) instead.

**Impact**:
- If backend instances have clock drift, breaker state transitions may be slightly inconsistent
- In practice, clock drift on cloud VMs is typically < 1 second due to NTP synchronization
- For cooldown periods of 1-48 hours, sub-second clock skew is negligible

**Mitigation**:
- Current implementation is functionally correct for single-instance deployments
- Multi-instance deployments should ensure NTP is enabled on all hosts
- A future enhancement could convert timestamps to use Firestore server timestamps

**Files Affected**:
- `YouTubeCircuitBreaker.java`: `openedAt`, `cooldownUntil`, `lastFailureAt` fields

### HALF_OPEN Probe Gating

**Fixed**: The circuit breaker's HALF_OPEN state now correctly gates requests to allow only a single probe.

**Implementation** (`YouTubeGateway.checkCircuitBreaker()`):
1. First calls `isOpen()` - handles OPEN state cooldown check and triggers OPEN→HALF_OPEN transition
2. If `isOpen()` returns `true`:
   - Check `getCurrentState()` - if HALF_OPEN, throw specific "probe already in progress" message
   - Otherwise (OPEN state with cooldown), throw generic "circuit is open" message
3. If `isOpen()` returns `false`:
   - State is either CLOSED (proceed) or HALF_OPEN with no probe yet
   - Call `allowProbe()` to atomically acquire probe permit using `compareAndSet(false, true)`
   - If `allowProbe()` returns false (race condition), throw "probe already in progress" message
4. Additional requests while probe is in progress receive: "circuit breaker is in HALF_OPEN state and probe already in progress"
5. On probe completion, the circuit breaker records success/failure and updates state accordingly

**Non-Rate-Limit Probe Failures**:
If a HALF_OPEN probe fails with a non-rate-limit error (network timeout, parsing error, etc.), the circuit must still clear the probe permit to avoid getting stuck. The `recordProbeFailure()` method handles this:
- Clears `probeInProgress` flag
- Reopens circuit at the **current** backoff level (does NOT increase backoff)
- Logs the failure as a transient error

This ensures the circuit can attempt another probe after cooldown expires, rather than being permanently stuck in HALF_OPEN with `probeInProgress=true`.

**Files Affected**:
- `YouTubeGateway.java`: `checkCircuitBreaker()`, `recordError()` methods
- `YouTubeCircuitBreaker.java`: `allowProbe()`, `isProbeRequest()`, `recordSuccess()`, `recordProbeFailure()` methods

### Integration Tests

The following integration tests have been added to verify circuit breaker and scheduler behavior:

1. **CircuitBreakerIntegrationTest.java**: Tests circuit breaker persistence with real Firestore
   - State survives restart
   - Multiple instances share state
   - Stale OPEN transitions to HALF_OPEN
   - Probe success/failure handling
   - Optimistic locking for concurrent updates

2. **SchedulerLockIntegrationTest.java**: Tests distributed lock for scheduler
...
Run integration tests with: `./gradlew test -Pintegration=true`

**Firestore Emulator Setup for Integration Tests**:

Integration tests require the Firestore emulator running with permissive rules. Test-specific configuration files are provided:

```bash
# Start emulator with test rules (from backend directory)
cd backend
...
- `firebase-test.json`: Firebase config pointing to permissive test rules
- `firestore-test.rules`: Allows all reads/writes for test isolation

The test rules (`firestore-test.rules`) allow unrestricted access:
```
rules_version = '2';
...
This avoids PERMISSION_DENIED errors that would occur with production rules.

### Unit Tests for Mid-Batch Behavior

**MidBatchSkippedItemsTest.java**: Tests ContentValidationService handling of skipped items
- Skipped items do NOT have status/lastValidatedAt updated
- Skipped count recorded in validation run details
- All items skipped when circuit breaker open at start
- Mixed results (valid, notFound, error, skipped) handled correctly
- Skipped items remain eligible for next validation run

This is implemented as a unit test (not integration test) because:
- It tests ContentValidationService logic using mocked YouTubeService
- It verifies behavior by asserting which videos are saved (vs skipped)
- No real Firestore or YouTube API calls needed
