...
- **Android Metadata Hydration** (Phase 8, 2025-10-07): `MetadataHydratorTest` exercises the real NewPipeExtractor client for list feeds, ensuring Albunyaan overrides win when set and extractor failures fall back without breaking Paging flows. Planned instrumentation (post-backend wiring) will: (1) throttle extractor responses to confirm cached items continue rendering while latency banners surface, (2) simulate offline/failure paths to verify fallback messaging and analytics are emitted, and (3) run TalkBack walkthroughs on hydrated list rows to ensure enriched descriptions remain accessible.
- **Android Stream Resolution** (Phase 8, 2025-10-07): `PlayerViewModelTest` drives the NewPipe-backed stream resolver with fake repositories, validating queue advancement, audio-only toggles, and error handling. Upcoming instrumentation will inject flaky network + Doze conditions to measure resolve latency, verify fallback messaging in the player banner, and confirm TalkBack announces the selected quality/audio-only state.
- **Android Downloads Queue** (Phase 9, 2025-10-07): WorkManager-backed `DownloadWorker` now resolves NewPipe streams, enforces a 500 MB quota, and notifies analytics; instrumentation covers pause/resume/cancel controls, TalkBack content descriptions, storage pressure, and now verifies metadata badges on the downloads list plus player offline indicators fed by `DownloadRepository` telemetry.
- **Cross-Team Follow-up**: Backend still needs to surface the EULA acceptance flag and accept telemetry payloads (see backlog items BE-DL-01/02); once available, extend instrumentation to validate manifest-driven gating and server acknowledgements. Legal sign-off on dialog copy tracked via LEGAL-01.
