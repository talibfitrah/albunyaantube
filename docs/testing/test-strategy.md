...
- **Android Filter Controls** (Phase 6 prep, 2025-10-04): `FilterManager` now persists selections via DataStore while exposing a `StateFlow` for UI bindings. Planned coverage: unit tests confirming filter combinations trigger expected state transitions, integration tests simulating chip interactions + dialog selections, instrumentation ensuring TalkBack announces filter changes, the clear button resets state, and process-death tests verifying selections survive restarts.
