<template>
  <section class="registry-card">
    <header class="card-header">
      <div>
        <h2>{{ t('registry.tabs.channels') }}</h2>
        <p>{{ t('registry.channels.description') }}</p>
      </div>
    </header>

    <div class="table-wrapper" role="region" aria-live="polite">
      <div v-if="error" class="error-state">
        <p>{{ t('registry.table.error', { resource: t('registry.tabs.channels') }) }}</p>
      ...
      <table v-else class="data-table">
        <thead>
          <tr>
            <th scope="col">{{ t('registry.channels.columns.channel') }}</th>
            <th scope="col">{{ t('registry.channels.columns.categories') }}</th>
            <th scope="col">{{ t('registry.channels.columns.subscribers') }}</th>
          </tr>
        </thead>
        <tbody>
          ...
          <tr v-else-if="!items.length">
            <td :colspan="3" class="empty-state">{{ t('registry.table.empty') }}</td>
          </tr>
          <tr v-for="channel in items" :key="channel.id">
            <td>
              <div class="entity">
                <img :src="channel.avatarUrl" :alt="channel.name" class="entity-thumb" />
                <div class="entity-meta">
                  <div class="entity-name">{{ channel.name }}</div>
                  <div class="entity-subtitle">{{ channel.ytId }}</div>
                </div>
              </div>
            </td>
            <td>
              <ul class="category-tags">
                <li v-for="tag in channel.categories" :key="tag.id" class="category-tag">{{ tag.label }}</li>
              </ul>
            </td>
            <td class="numeric">{{ formatNumber(channel.subscriberCount, currentLocale) }}</td>
          </tr>
        </tbody>
      </table>
    </div>
...
import { fetchChannelsPage } from '@/services/registry';
import { formatNumber as formatNumberUtil } from '@/utils/formatters';
import type { ChannelSummary } from '@/types/registry';

const { t, locale } = useI18n();
const currentLocale = computed(() => locale.value);

const pagination = useCursorPagination<ChannelSummary>((cursor, limit) =>
  fetchChannelsPage({ cursor, limit })
);
