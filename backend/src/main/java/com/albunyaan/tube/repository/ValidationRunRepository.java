...
public class ValidationRunRepository {
    ...
    public ValidationRunRepository(Firestore firestore) {
    ...
    public ValidationRun save(ValidationRun validationRun) throws ExecutionException, InterruptedException {
    ...
    public Optional<ValidationRun> findById(String id) throws ExecutionException, InterruptedException {
     ...
     */
    public Optional<ValidationRun> findLatest() throws ExecutionException, InterruptedException {
        ApiFuture<QuerySnapshot> query = getCollection()
                .orderBy("startedAt", Query.Direction.DESCENDING)
                .limit(1)
                .get();

        List<ValidationRun> runs = query.get().toObjects(ValidationRun.class);
